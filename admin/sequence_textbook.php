...
<head>
    ...
    <title>Sequence Textbook | Wayshk Admin</title>
    <!-- Datatable -->
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                ...
                <div class="col-4 text-center">
                    <ul id="sortable-list">
                        <?php
                        $fetch_course = $db_handle->runQuery("SELECT * FROM `textbook` ORDER BY order_no");
                        $no_fetch_course = $db_handle->numRows("SELECT * FROM `textbook` ORDER BY order_no");
                        for ($i = 0; $i < $no_fetch_course; $i++){
                            ?>
                            <li data-id="<?php echo $fetch_course[$i]['id'];?>"><?php echo $fetch_course[$i]['order_no'];?> - <?php echo $fetch_course[$i]['textbook_title'];?></li>
                            <?php
                        }
                        ?>
                    </ul>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        async function saveOrderToBackend(newOrder) {
            try {
                const response = await fetch("save_textbook_order.php", {
                    method: "POST",
                    headers: {
