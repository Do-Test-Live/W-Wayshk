            ...
            <div class="row">
                ...
                <?php } else { ?>
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Defected Products</h4>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display min-w850">
                                        ...
                                        <tbody>
                                        <?php
                                        $defected_products = $db_handle->runQuery("SELECT product.p_name,defected_products.quantity,defected_products.note,defected_products.inserted_at FROM `defected_products`,`product` WHERE defected_products.id = product.id order by defected_products.id desc");
                                        $row_count = $db_handle->numRows("SELECT product.p_name,defected_products.quantity,defected_products.note,defected_products.inserted_at FROM `defected_products`,`product` WHERE defected_products.id = product.id order by defected_products.id desc");

                                        for ($i = 0; $i < $row_count; $i++) {
                                            ?>
                                            <tr>
                                                <td><?php echo $i + 1; ?></td>
                                                <td><?php echo $defected_products[$i]['p_name']; ?></td>
                                                <td><?php echo $defected_products[$i]['quantity']; ?></td>
                                                <?php
                                                $date = date_create($defected_products[$i]['inserted_at']);
                                                $date_formatted = date_format($date, "d F y, g:i A");
                                                ?>
                                                <td><?php echo $date_formatted; ?></td>
                                                <td><?php echo $defected_products[$i]['note']; ?></td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
